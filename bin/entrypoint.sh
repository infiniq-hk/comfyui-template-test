...
if [[ -n "${CHECKPOINT_IDS_TO_DOWNLOAD:-}" || -n "${LORAS_IDS_TO_DOWNLOAD:-}" || -n "${VAE_IDS_TO_DOWNLOAD:-}" || -n "${CONTROLNET_IDS_TO_DOWNLOAD:-}" || -n "${EMBEDDING_IDS_TO_DOWNLOAD:-}" || -n "${UPSCALER_IDS_TO_DOWNLOAD:-}" ]]; then
